...
contract `loot_registry` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CDRNHQ7ZMBZEAZD5F3NTXZTVPSZ4NMTHLCDX746SAVNKNALLXJZKB3UQ \
  --output-dir ./path/to/loot_registry
```
...
```json
"dependencies": {
  "loot_registry": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url http://localhost:8000/rpc --network-passphrase \"Standalone Network ; February 2017\" --id CDRNHQ7ZMBZEAZD5F3NTXZTVPSZ4NMTHLCDX746SAVNKNALLXJZKB3UQ --name loot_registry"
}
```
...
```js
import { Contract, networks } from "loot_registry"
